# -------------------------------------------------------------------------
# @Description: brms phylogenetic SEM construction (without MRT)
# -------------------------------------------------------------------------
...
# Loading CSR data
checklist.csr <- read_csv("./data/checklist_accepted_matches_CSR_v4.20251023.csv", show_col_types = FALSE)

table(checklist.csr$keep)

str(checklist.csr)
glimpse(checklist.csr)

checklist.csr.CHN.mrt <- checklist.csr %>%
  select(taxon_name, accepted_plant_name_id, invasion_status, TPL_names, c_score, s_score, r_score, wcup_eco_use_num_2out)

checklist.csr.CHN.mrt01 <- checklist.csr.CHN.mrt %>%
  mutate_at(vars(c_score:wcup_eco_use_num_2out), ~ as.numeric(.)) %>%
  mutate(tip_label = str_replace(taxon_name, " ", "_"))

str(checklist.csr.CHN.mrt01)

# ==========================================================================
# Get non_naturalized vs naturalized data (1 vs 2+3)
# ==========================================================================
...
c_formula_nat <- brms::bf(invasion_status ~ c_score.scaled + eco.use.scaled + (1|gr(species, cov = phy.m.corr.12b)), family = bernoulli(link = 'logit'))

s_formula_eco.use <- brms::bf(eco.use.scaled ~ s_score.scaled + (1|gr(species, cov = phy.m.corr.12b)), family = gaussian())
s_formula_nat <- brms::bf(invasion_status ~ s_score.scaled + eco.use.scaled + (1|gr(species, cov = phy.m.corr.12b)), family = bernoulli(link = 'logit'))

r_formula_eco.use <- brms::bf(eco.use.scaled ~ r_score.scaled + (1|gr(species, cov = phy.m.corr.12b)), family = gaussian())
r_formula_nat <- brms::bf(invasion_status ~ r_score.scaled + eco.use.scaled + (1|gr(species, cov = phy.m.corr.12b)), family = bernoulli(link = 'logit'))

# Fit model
c_model_nat <- brm(c_formula_eco.use + c_formula_nat + set_rescor(FALSE),
...
# Fit model
s_model_nat <- brm(s_formula_eco.use + s_formula_nat + set_rescor(FALSE),
  data = checklist.csr.CHN.mrt.12b,
  data2 = list(phy.m.corr.12b = phy.m.corr.12b),
  control = list(adapt_delta = 0.99, max_treedepth = 15),
  chains = 4,
...
# Fit model
r_model_nat <- brm(r_formula_eco.use + r_formula_nat + set_rescor(FALSE),
  data = checklist.csr.CHN.mrt.12b,
  data2 = list(phy.m.corr.12b = phy.m.corr.12b),
  control = list(adapt_delta = 0.99, max_treedepth = 15),
  chains = 4,
...
brms::loo(c_model_nat)
brms::loo(s_model_nat)
brms::loo(r_model_nat)

brms::bayes_R2(c_model_nat)
brms::bayes_R2(s_model_nat)
brms::bayes_R2(r_model_nat)
...
c_formula_ias <- brms::bf(invasion_status ~ c_score.scaled + eco.use.scaled + (1|gr(species, cov = phy.m.corr.23b)), family = bernoulli(link = 'logit'))

s_formula_eco.use <- brms::bf(eco.use.scaled ~ s_score.scaled + (1|gr(species, cov = phy.m.corr.23b)), family = gaussian())
s_formula_ias <- brms::bf(invasion_status ~ s_score.scaled + eco.use.scaled + (1|gr(species, cov = phy.m.corr.23b)), family = bernoulli(link = 'logit'))

r_formula_eco.use <- brms::bf(eco.use.scaled ~ r_score.scaled + (1|gr(species, cov = phy.m.corr.23b)), family = gaussian())
r_formula_ias <- brms::bf(invasion_status ~ r_score.scaled + eco.use.scaled + (1|gr(species, cov = phy.m.corr.23b)), family = bernoulli(link = 'logit'))

# Fit model
c_model_ias <- brm(c_formula_eco.use + c_formula_ias + set_rescor(FALSE),
...
# Fit model
s_model_ias <- brm(s_formula_eco.use + s_formula_ias + set_rescor(FALSE),
  data = checklist.csr.CHN.mrt.23b,
  data2 = list(phy.m.corr.23b = phy.m.corr.23b),
  control = list(adapt_delta = 0.99, max_treedepth = 15),
  chains = 4,
...
# Fit model
r_model_ias <- brm(r_formula_eco.use + r_formula_ias + set_rescor(FALSE),
  data = checklist.csr.CHN.mrt.23b,
  data2 = list(phy.m.corr.23b = phy.m.corr.23b),
  control = list(adapt_delta = 0.99, max_treedepth = 15),
  chains = 4,
...
brms::loo(c_model_ias)
brms::loo(s_model_ias)
brms::loo(r_model_ias)

brms::bayes_R2(c_model_ias)
brms::bayes_R2(s_model_ias)
brms::bayes_R2(r_model_ias)
...
save(c_model_nat,
     ...
     s_model_ias,
     r_model_ias,
     file = "./results/csr.brm_phylo.sem_models(AllSpWithoutMRT).Rdata")
...
write_csv(model_nat.tidy02, "./results/phylo.sem.model_nat.tidy02(AllSpWithoutMRT).csv")
...
write_csv(model_ias.tidy02, "./results/phylo.sem.model_ias.tidy02(AllSpWithoutMRT).csv")
